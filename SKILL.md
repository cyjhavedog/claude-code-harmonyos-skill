...
### @kit.ConnectServiceKit（MQTT）

```typescript
import { connect } from '@kit.ConnectServiceKit';

let client = await connect.mqttConnect({
  serverAddress: 'broker.emqx.io',
  serverPort: 1883,
  clientId: 'harmony_client_001'
})

client.subscribe('device/sensor/#')

client.publish('device/sensor/data', JSON.stringify({
  temp: 26.5, humi: 65.2
}), { qos: 1, retain: false })

client.on('message', (topic: string, payload: string) => {
  hilog.info(0x0000, 'MQTT', `收到 [${topic}]: ${payload}`)
})
...
|-----------|-------------------|
| API 20 | ArkTS 12 跨包类型、ArkWeb chromium 132、MQTT 增强、传感器增强 |
...
# 启动
hdc shell aa start -a EntryAbility -b com.example.mqtt
